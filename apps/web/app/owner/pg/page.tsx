...
export default function PGListingForm() {
  ...
  const onSubmit = async (data: FormData) : Promise<void> => {
    ...
    try {
      ...
      const response = await axios.post("http://http-server.roomlocus.com/api/v1/owner/pg",
        formData,
        {
          headers: {
            'token': token,
      ...
      if (response.status === 201) {
        localStorage.setItem("pgId", response.data.pg.id);
        router.push("/owner/pg/images");
      }
    } catch (error) {
